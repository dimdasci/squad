...
output=$(run_claude "In the product-brief skill, what are the main steps in order? List them briefly." 30)
...
output=$(run_claude "In the product-brief skill, how should problem framing questions be asked? Should the agent use multiple-choice or open-ended questions?" 30)

assert_contains "$output" "open-ended\|open ended" "Mentions open-ended questions" || exit 1
assert_contains "$output" "not.*multiple.choice\|no.*multiple.choice\|avoid.*multiple.choice\|NOT.*predefined\|not.*predefined" "Warns against multiple-choice" || exit 1
...
output=$(run_claude "In the product-brief skill, how does the independent review work? Does the reviewer see the conversation history?" 30)

assert_contains "$output" "fresh\|independent\|fork\|separate\|isolated" "Review uses fresh context" || exit 1
assert_contains "$output" "product-brief-review" "Names the review skill" || exit 1
...
output=$(run_claude "In the product-brief skill step 9, when the review returns FAIL, what are the different ways the agent can respond to each finding?" 30)
...
output=$(run_claude "Where does the product-brief skill save the brief artifact? What is the file path?" 30)
